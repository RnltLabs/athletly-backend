...
class GateContext:
    ...
    # Sprint Q: the most recent save_plan tool result (the canonical
    # plan structure the agent persisted this turn). Empty dict when
    # no save_plan ran this turn. The ``post_save_plan_grounded`` gate
    # uses this as the source of truth when checking whether session
    # specifics in the response are real or fabricated.
    saved_plan: dict = field(default_factory=dict)
...
# Gate 6: post_save_plan_grounded ------------------------------------------
#
# After save_plan returns successfully, the agent's reply must reference
# only sessions actually in the saved plan. The Iter 3 persona failure
# pattern: response says "morgen 8km bei 4:30/km" but the saved plan's
# week-1 day-1 is "easy 60min" with no such pace. The gate inspects
# session-detail tokens (distance + pace pair within 50 chars) and
# verifies each pair appears in plan_snapshot.sessions.
#
# Detection is intentionally conservative: only fires when BOTH a
# distance (e.g. "8km") and a pace (e.g. "4:30/km") appear together,
# because those are the high-confidence fabrications. A standalone
# phase summary ("Woche 1 ist Base-Phase, locker einlaufen") is
# allowed to pass without matching anything specific.

# Distance token: "8km", "8 km", "8,5 km", "8.5km"
_PSG_DISTANCE_RE: re.Pattern[str] = re.compile(
    rf"\b{_NUM_ATOM}\s*km\b", re.IGNORECASE,
)

# Pace token: "4:30/km", "4,30/km", "4:30 /km" (with optional space).
# We deliberately do NOT match decimal-form paces ("4,5 min/km") here:
# those almost never co-occur with a fabricated distance in the
# observed failure cases, and including them would widen the false
# positive surface. The mm:ss form is the canonical pace surface
# coaches use.
_PSG_PACE_RE: re.Pattern[str] = re.compile(
    r"\b(\d{1,2})[:,](\d{2})\s*/\s*km\b",
)

# Max characters between a distance token and a pace token for them
# to count as a "session-detail pair". The Iter 3 fabrication pattern
# was "8km bei 4:30/km" or "8 km mit 4:30/km" - the connector is at
# most 5-10 chars. 50 chars is generous but bounded so two unrelated
# numbers in the same paragraph do not pair up.
_PSG_PAIR_MAX_DISTANCE: int = 50


def _normalize_pace_token(minutes: str, seconds: str) -> str:
    """Return the canonical ``mm:ss`` form of a pace.

    Both ``5:30`` (canonical) and ``5,30`` (German user typed comma
    where a colon belongs, e.g. ``5,30/km``) must collapse to the
    same key so a response saying ``5,30/km`` can match a plan
    description saying ``5:30/km``.
    """
    return f"{int(minutes)}:{seconds.zfill(2)}"


def _normalize_distance_token(token: str) -> str:
    """Return a canonical numeric key for a distance string.

    ``8``, ``8.0``, ``8,0`` all collapse to ``8``. ``8,5`` and ``8.5``
    both collapse to ``8.5``. Used so the pair-equality check is robust
    against the German comma decimal AND across plain integer vs
    decimal forms.
    """
    try:
    ...
    # Strip trailing zeros after the decimal point so 8.50 -> 8.5.
    return f"{value:g}"


def _extract_session_pairs(text: str) -> list[tuple[str, str, str]]:
    """Find (distance, pace) pairs within ``_PSG_PAIR_MAX_DISTANCE`` chars.

    Returns a list of (normalized_distance, normalized_pace, raw_span)
    tuples for each pair detected. ``raw_span`` is the verbatim text
    slice and is used in the gate's failure reason for debuggability.
    """
    if not text:
        return []
    distances = [
        (m.start(), m.end(), m.group())
        for m in _PSG_DISTANCE_RE.finditer(text)
    ]
    paces = list(_PSG_PACE_RE.finditer(text))
    if not distances or not paces:
        return []

    pairs: list[tuple[str, str, str]] = []
    for d_start, d_end, d_raw in distances:
        d_token = d_raw.lower().replace("km", "").strip()
        d_key = _normalize_distance_token(d_token)
        for p_match in paces:
            p_start = p_match.start()
            p_end = p_match.end()
            # Pair if pace is within window either before or after the
            # distance. Use the closer edge to measure the gap.
            if p_start >= d_end:
                gap = p_start - d_end
            elif p_end <= d_start:
                gap = d_start - p_end
            else:
                gap = 0  # overlap (extremely rare, accept)
            if gap > _PSG_PAIR_MAX_DISTANCE:
                continue
            p_key = _normalize_pace_token(p_match.group(1), p_match.group(2))
            span_start = min(d_start, p_start)
            span_end = max(d_end, p_end)
            pairs.append((d_key, p_key, text[span_start:span_end]))
    return pairs


def _session_text_blob(saved_plan: dict) -> str:
    """Flatten plan_snapshot.sessions to a single searchable string.

    Concatenates every text field of every snapshot session so the
    gate can substring-match against ``name``, ``description``,
    ``notes`` and any duration/intensity hints. Returns the empty
    string when ``saved_plan`` lacks a ``sessions`` list.
    """
    if not isinstance(saved_plan, dict):
        return ""
    snapshot = saved_plan.get("plan_snapshot") or saved_plan
...
def _pair_appears_in_plan(
    distance_key: str, pace_key: str, plan_blob: str,
) -> bool:
    """Return True iff both the distance and pace appear in ``plan_blob``.

    The plan blob has been lowercased upstream. We accept either the
    canonical mm:ss form (``5:30``) or the comma form (``5,30``) for
    the pace so plans authored with either style match. The distance
    must appear as a token (followed by ``km`` or whitespace boundary)
    so ``8`` does not match a ``180`` heart rate elsewhere in the plan.
    """
    pace_alt = pace_key.replace(":", ",")
    if pace_key not in plan_blob and pace_alt not in plan_blob:
        return False
    # Cheap distance check: look for "<dist>km" or "<dist> km" forms.
    dist_compact = f"{distance_key}km"
    dist_spaced = f"{distance_key} km"
    return dist_compact in plan_blob or dist_spaced in plan_blob


def _gate_post_save_plan_grounded(ctx: GateContext) -> GateResult:
    """Gate 6: post-save_plan reply must not fabricate session specifics.

    Fires only when ``save_plan`` was called this turn AND the response
    contains a (distance, pace) pair within 50 chars. For each pair we
    check the saved plan_snapshot's session text; if the pair is not
    present, the gate fails. Phase-level summaries that mention
    neither a distance nor a pace are allowed to pass.
    """
    if "save_plan" not in set(ctx.tools_called_this_turn):
        return GateResult(passed=True, gate_id="post_save_plan_grounded")

    if not ctx.response_text:
        return GateResult(passed=True, gate_id="post_save_plan_grounded")

    pairs = _extract_session_pairs(ctx.response_text)
    if not pairs:
        return GateResult(passed=True, gate_id="post_save_plan_grounded")

    plan_blob = _session_text_blob(ctx.saved_plan)
    unsupported: list[str] = []
    for d_key, p_key, raw in pairs:
        if not _pair_appears_in_plan(d_key, p_key, plan_blob):
            unsupported.append(f"{d_key}km @ {p_key}/km ({raw.strip()!r})")
    ...
    return GateResult(
        passed=False,
        gate_id="post_save_plan_grounded",
        reason=(
            "post-save_plan response references session details not in "
            f"the saved plan: {first}"
        ),
        required_action=(
            "SYSTEM CHECK: After save_plan, your reply mentioned a "
            "specific session (Distanz + Pace) that is NOT in the "
            "saved plan_snapshot. Do not fabricate distances or paces. "
            "Either quote the actual session fields from "
            "plan_snapshot.sessions or keep the reply at the phase / "
            "week-summary level. Rewrite without inventing specifics."
        ),
    )
...
GATES: tuple[Gate, ...] = (
    ...
    Gate(
        ...
        "response_gate_post_save_plan_grounded",
