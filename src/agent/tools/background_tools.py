...
def register_background_tools(registry: ToolRegistry, user_model) -> None:
    ...
    registry.register(Tool(
        ...
        description=(
            "Fork a long operation into the background; returns job_id "
            "immediately. Harvest via check_background_job/list_my_jobs. Use "
            "only when work would block >30s AND user signalled patience; "
            "default to synchronous when user is actively waiting. Kinds: "
            "'subagent' (web research, async spawn_subagent), 'macrocycle' "
            "(args: name,weeks,start_date,periodization_model), 'plan' (args: "
            "focus,macrocycle_week,feedback,sport_distribution; task used as "
            "focus fallback). On rejection: do not fabricate job_id; fall back "
            "to sync call."
        ),
